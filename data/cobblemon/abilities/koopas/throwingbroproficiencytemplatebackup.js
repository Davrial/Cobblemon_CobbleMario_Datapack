{
  ...
  onModifyMove(move, pokemon) {
    ...
    if (move.type === "Fighting" || move.type === "Flying") {
      move.forceSTAB = true;
    }
  },
  ...
  onResidual(pokemon) {
      ...
      if (this.effectState.flingItems.includes(pokemon.lastItem) && !pokemon.item && pokemon.lastMove.id === "fling" && checkStatesIncludes) {
        this.hint(`flingItemsCheck number: ${flingItemsCheck}` );
        this.hint(`checkStatesIncludes result: ${checkStatesIncludes}` );
        this.hint(`checkStates.includes result: ${checkStates.includes(flingItemsCheck)}` );
